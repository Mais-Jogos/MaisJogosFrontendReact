...
const Card = ({game, dispatch, cart}) => {
  ...
  <>
    ...
    <div className="card">
      <Link to={`/jogos/${game?.name?.toLowerCase().replace(/ /g,"-")}`} key={game?.id}  aria-label={game?.name?.toLowerCase().replace(/ /g,"-")}>
        <div className="card__image">
          <img src={game?.background_image} alt={game?.name} />
        </div>
      </Link>
      <Link to={`/jogos/${game}`} key={game?.id} aria-label={game?.name?.toLowerCase().replace(/ /g,"-")}> 
        <h2>{game?.name}</h2>
      </Link>
